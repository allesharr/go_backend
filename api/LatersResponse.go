...
func CalcLaters(engine *xorm.Engine, current_date string) []LaterPerson {
	names_of_employers = []string{}
	...
	// MUY IMPORTANTE PARA NUEVOS
	//Si queres usar Close - no puedes usar eso
	// defer engine.Close()
	...
	for _, name := range names_of_employers {
		...
		later := Morning_OR_Evining_Late(data_events, name, current_date, Start_Day_Time, End_Day_Time)
...
func Find_All_Events_In_Day_Por_Name(x *xorm.Engine, day string, name string) []User_Event_Per_Day {
	...
	if err != nil {
		fmt.Println("Error while fetching data (custom)", err)
	}
...
func Morning_OR_Evining_Late(data_events []User_Event_Per_Day, name, current_date string, Start_Day_Time, End_Day time.Time) LaterPerson {
	lp := LaterPerson{
		Name:             name,
		IsMorningLate:    false,
